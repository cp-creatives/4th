...
class Parallax {
  ...
  onAnimationFrame() {
    ...
    let active = Math.abs(this.velocityX - this.motionX) > 0.05 ||
                 Math.abs(this.velocityY - this.motionY) > 0.05
    for (let index = 0; index < this.layers.length; index++) {
      let layer = this.layers[index],
          ...
          lastOffset = this.lastOffsets[index] || {x: 0, y: 0}
      if (Math.abs(xOffset - lastOffset.x) > 0.05 || Math.abs(yOffset - lastOffset.y) > 0.05) {
        active = true
      }
      this.setPosition(layer, xOffset, yOffset)
      this.lastOffsets[index] = {x: xOffset, y: yOffset}
    }
    this.raf = active ? rqAnFr(this.onAnimationFrame) : null
  }
